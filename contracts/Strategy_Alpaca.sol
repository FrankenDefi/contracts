...
interface IHelp {
    function Bytecode_0_1_2() external view returns (bool);
}
...
contract Strategy_Alpaca is StrategyV3 {
    ...
    function _unfarm(uint256 _wantAmt) internal override {
        if (_wantAmt == 0) {
            IAlpacaFarm(farmContractAddress).harvest(pid);
        } else {
    ...
    function earn() public override nonReentrant whenNotPaused {
        ...
        if (isSingleVault) {
            ...
            IERC20(token0Address).safeIncreaseAllowance(vaultAddress, preToken0Amt);
            IVault(vaultAddress).deposit(preToken0Amt);
